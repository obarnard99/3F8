...
def compute_average_ll(X, y, w):
    """Computes a 1d array with the average likelihood per data point and dimension equal to the number of training
    epochs """
    # X: 2d array with the input features
    # y: 1d array with the class labels (0 or 1)
...
def plot_predictive_distribution(X, y, predict):
    # X: 2d array with the input features
    # y: 1d array with the class labels (0 or 1)
    # predict: function that recives as input a feature matrix and returns a 1d vector with the probability of class 1.
    xx, yy = plot_data_internal(X, y)
    ax = plt.gca()
...
def predict_for_plot_expanded_features(x):
    # x: 2d array with input features at which to compute the predictions using the feature expansion (uses parameter
    # vector w and the 2d array X with the centers of the basis functions for the feature expansion,
    # which are defined outside the function's scope)
